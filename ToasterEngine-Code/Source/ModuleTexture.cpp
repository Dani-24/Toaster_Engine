...
GLuint ModuleTexture::LoadToMemory(char* buffer, int size, int* w, int* h)
{
	ILuint imageID;
...
void ModuleTexture::SaveDDS(char* buffer, int size, const char* fileName)
{
	ILuint imageID;
	ilGenImages(1, &imageID);
	ilBindImage(imageID);
	...
	if (_size > 0)
	{
		...
		app->importer->Save(path.c_str(), (char*)data, _size);
...
void ModuleTexture::Import(char* buffer, int bSize, Resource* res)
{
